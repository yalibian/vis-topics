(function() {
			...
			.on("drag", function(d) {
				...
				if (d.layer_id <= layers.length-1 && d.layer_id >= 0) {
					...
					if (d.layer_id >= layers.length-1) {
						...
						threshold_down = (d3.max(layer_down.__data__, function(d){return d.y0;}) + d3.min(layer_down.__data__, function(d){return d.y0+d.y;}))/2;
					
					} else if (d.layer_id <= 0){
						var threshold_up = (d3.max(layer_up.__data__, function(d){return d.y0;}) + d3.min(layer_up.__data__, function(d){return d.y0+d.y;}))/2,
	...
	function stackTopics(topics) {
		...
		var layers = stack(topics.slice(0,topicSize).map(function(d) { return bumpLayer(d.periods); }));
		// console.log(layers);
		// normalize the layer
		...
		// console.log(maxHigh);
		// Math.max(layers[topicSize-1],function())
		// return layers;
		// console.log(layers);	
		...
		return layers;
		//
		// var maxWeight = d3.max(layers(len(layers)-1));
		// max(layer.map(functo))
	}
	function bumpLayer( periods ) {
			//alert(periods);
		var temp = periods.map( function(d, i) { return { x: i, y: d.weight }; } );
		var arr = new Array();
		...
		// console.log(arr);
		return arr;
		// return periods.map( function(d, i) { return { x: i, y: d.weight }; } );
	}
